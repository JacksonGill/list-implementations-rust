...
impl<T> LinkedList<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        ...
        unsafe {
            ...
            if let Some(old) = self.front {
                ...
                // If there's no front, then we're thje empty list and neeed to set the back too
                self.back = Some(new);
            }
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        unsafe {
            self.front.map(|node| {
                // Bring the box back to life so we can move out its value and Drop it
                ...
                self.len -= 1;
                result
                // boxed_node gets implicitly freed here
            })
        }
    }
    ...
    pub fn pop_back(&mut self) -> Option<T> {
        unsafe {
            self.back.map(|node| {
                let boxxed_node = Box::from_raw(node.as_ptr());
                let result = boxxed_node.elem;

                self.back = boxxed_node.front;
            ...
            unsafe {
                ...
                if self.cur.is_some() {
                    *self.index.as_mut().unwrap() += 1;
                    ...
                    // walk to the ghost, no more index
                    self.index = None;
                }
            }
